...
- An expression of type other than *array_type* or `string`, provided the type implements an accessible method named `GetPinnableReference`, that method has no parameters, that method returns a `ref` or `ref readonly` variable of an *unmanaged_type* `T`, and `T*` is implicitly convertible to the pointer type given in the `fixed` statement. In this case, the initializer computes the address of the returned variable, and that variable is guaranteed to remain at a fixed address for the duration of the `fixed` statement.
